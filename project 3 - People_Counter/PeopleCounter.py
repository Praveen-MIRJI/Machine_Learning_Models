...
while True:
    ...
    for result in resultsTracker:
        ...
        if limitsUp[0] <cx <limitsUp[2] and limitsUp[1]-3 < cy < limitsUp[1]+3:
            if totalCountUp.count(id) ==0:
                totalCountUp.append(id)
                cv2.line(img, (limitsUp[0], limitsUp[1]), (limitsUp[2], limitsUp[3]), (0, 0, 255), 5)

        if limitsDown[0] < cx < limitsDown[2] and limitsDown[1] - 3 < cy < limitsDown[1] + 3:
    ...
    cv2.putText(img, str(len(totalCountDown)), (1191, 345), cv2.FONT_HERSHEY_PLAIN, 5, (50, 50, 238), 7)
